...
def compare_all_different(list1, list2):
    ...
    if count(list1) != count(list2):
        ...
        for dict1 in list1:
            ...
            total_population = dict1['总人口']  # 总人口
            city_population = dict1['城镇人口'] # 城镇人口
            rural_population = dict1['乡村人口']  # 乡村人口
            # 通过一个字段标志另一个列表中的唯一字典
            dict2 = get_dict_wih_same_key(new_list1=[year, country_name, total_population, city_population,rural_population], list2=list2)
            # 接下来就是两条字典数据比对
            try:
...
def get_dict_wih_same_key(new_list1, list2):
    i = 0
    for dict2 in list2:
        if dict2['年份'] == new_list1[0] and dict2['国家名称'] == new_list1[1]:
                return dict2
...
if __name__ == '__main__':
    ...
    my_sheet_name = 'test_country_year_population'
    list1 = read_excel_into_list(my_excel_file_path, my_sheet_name)
    my_json_file_path = '../../../resource/ads_country_year_population.json'
