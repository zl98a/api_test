...
def compare_all_different(list1, list2):
    ...
    if count(list1) != count(list2):
        ...
        for dict1 in list1:
            ...
            urban_population = dict1['乡村人口']  # 乡村人口
            # 通过一个字段标志另一个列表中的唯一字典
            dict2 = get_dict_wih_same_key(new_list1=[year, province_name, total_population, city_population, urban_population], list2=list2)
            # 接下来就是两条字典数据比对
            try:
...
if __name__ == '__main__':
    ...
    my_sheet_name = 'test_year_province_population'
    list1 = read_excel_into_list(my_excel_file_path, my_sheet_name)
    my_json_file_path = '../../../resource/ads_year_province_population.json'
